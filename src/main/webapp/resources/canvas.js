...
function drawArea(radius) {
    ...
    ctx.textBaseline = 'middle'
    ctx.fillStyle = '#000'
    ctx.fillText("R", 0, -radius);
    ctx.fillText("R/2", 0, -radius/2);
    ctx.fillText("-R", 0, radius);
    ctx.fillText("-R/2", 0, radius/2);
    ctx.fillText("R", radius, 0);
    ctx.fillText("R/2", radius/2, 0);
    ctx.fillText("-R", -radius, 0);
    ctx.fillText("-R/2",-radius/2, 0);
    ctx.stroke();
    ctx.scale(-1,1);
...
canvas.addEventListener('click', function(event) {
    ...
    document.getElementById("chart-form:chart-r").value = r
    ...
    function tempik() {
        ...
        if (r === "1" || r === "2" || r === "3" || r === "4" || r === "5") {
            ...
            console.log("R по необъяснимым вселенной причинам выходит за рамки одз")
        }
    }
